...
contract Blackjack is IBlackjack, Ownable {
    ...
    function randNumber() private returns (uint256 num) {
        num = uint256(keccak256(abi.encodePacked(block.timestamp, seed))) % 52;
        seed++;
    }
    ...
    function checkHands(uint256 card, uint256[] storage hand) private view returns (bool) {
    ...
    function calculatePayout(Game storage game) private view returns (uint256 payout) {
